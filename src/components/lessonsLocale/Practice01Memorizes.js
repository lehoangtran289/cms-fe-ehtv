...
const Practice01Memorizes = ({ config }) => {
    ...
    const handleSubmit = () => {
      ...
      const requestData = data.map((item) => {
        return {
          'practiceId': item.id,
          'content': item.contentTranslate ? item.contentTranslate.trim() : null,
          'type': 'translated'
        };
      });
      const requestBody = {
                ...
                <Grid container spacing={1} wrap='wrap'>
                  ...
                  <Grid item lg={5} md={5} sm={5} className={classes.dialog}>
                    <TextField
                      ...
                      variant='outlined'
                      defaultValue={item.contentTranslate}
                      onBlur={(event) => handleDialogTranslateChange(event, index)}
                    />
                  </Grid>
                </Grid>
                <Divider sx={{
...
export default Practice01Memorizes;
